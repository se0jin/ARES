...
- 실외 기상
  - 기상청 초단기실황 API (천안 nx=63, ny=89)

- 저장
  - DB1 sensor_data   (co2 포함)
  - DB2 sensor_data_2 (co2 제외)
...
def read_co2():
    try:
        ...
        time.sleep(0.#!/usr/bin/env python3
"""
...
def job():
    ...
    row2 = {
        **base_row,
        ...
        "co2_predicted": None,
        "co2_source":    "sensor" if (indoor and indoor.get("co2_in") is not None) else "fallback",
    }
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(f"[DEBUG] CO2 응답길이: {len(resp)}, hex: {resp.hex()}")  # 추가

        if len(resp) < 9:
            return None

        co2 = (resp[2] << 8) | resp[3]
        print(f"[DEBUG] CO2 값: {co2} ppm")  # 추가
        return co2
...
def fetch_weather():
    ...
    try:
        ...
        if "body" not in data["response"]:
            ...
            return None
        items = data["response"]["body"]["items"]["item"]
        weather = {"temp_out": None, "rain_out": None, "wind_out": None, "solar_out": None}
        for item in items:
            cat = item["category"]
            val = item["obsrValue"]
            if cat == "T1H":
                ...
                weather["rain_out"] = 0 if val == "강수없음" else float(val)
            elif cat == "WSD":
                weather["wind_out"] = float(val)
            elif cat == "SI":
                weather["solar_out"] = float(val)
...
def job():
    ...
    # DB1 (CO2 포함, 1시간 주기)
    row1 = base_row.copy()
    row1["co2_in"] = indoor.get("co2_in")

    # DB2 (CO2 2시간마다 짝수 시간에만 저장)
    row2 = base_row.copy()
    row2["co2_in"] = indoor.get("co2_in") if datetime.now().hour % 2 == 0 else None

    # DB3 (CO2 포함, 1시간 주기)
    row3 = {
        "datetime": dt,
        "co2_in":   indoor.get("co2_in")
    }

    # 저장 실행
    try:
        supabase.table("sensor_data").insert(row1).execute()
        # 데이터가 어떻게 들어갔는지 JSON 형태로 출력
        print(f"[SUCCESS] DB1 저장 완료: {json.dumps(row1, indent=2, ensure_ascii=False)}")
    except Exception as e:
        print(f"[ERROR] DB1 저장 실패: {e}")

    try:
        supabase2.table("sensor_data_2").insert(row2).execute()
        print(f"[SUCCESS] DB2 저장 완료: {json.dumps(row2, indent=2, ensure_ascii=False)}")
    except Exception as e:
        print(f"[ERROR] DB2 저장 실패: {e}")

    try:
        supabase3.table("sensor_data_3").insert(row3).execute()
        print(f"[SUCCESS] DB3 저장 완료: {json.dumps(row3, indent=2, ensure_ascii=False)}")
    except Exception as e:
        print(f"[ERROR] DB3 저장 실패: {e}")
