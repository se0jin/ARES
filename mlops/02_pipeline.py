...
# ※ RETRAIN_R2_THRESHOLD: 모니터링 R² 이 이 값 이하이면 재학습 트리거
# ※ -1.0 = 비활성화 / 0.65 = 활성화 권장값
RETRAIN_R2_THRESHOLD    = 0.65

# ── DB1 데이터 품질 검증 기준 ────────────────────────────────────
# DB1 co2_in 이상값 필터링: 이 범위 밖 행은 재학습 데이터에서 제외
CO2_VALID_MIN           = 300    # ppm — 이하는 센서 오류로 판단
CO2_VALID_MAX           = 2000   # ppm — 이상은 센서 오류로 판단
# co2_in Null 비율이 이 이상이면 재학습 데이터 품질 불량으로 스킵
CO2_NULL_RATIO_LIMIT    = 0.30   # 30% 이상 Null → 재학습 스킵
# 재학습에 사용할 DB1의 최신 N행만 사용 (0 = 전체 사용)
RETRAIN_RECENT_ROWS     = 0      # 0 = 전체
...
def run_monitoring(db2: Client, db3: Client,
    ...
    actual_df["dt_hour"] = (
        pd.to_datetime(actual_df["datetime"], format="mixed", utc=True)
        .dt.floor("h")
    )
...
def run_retrain(db1: Client, trigger_reason: str = "manual") -> lgb.LGBMRegressor | None:
    ...
    try:
        ...
        if train_df is None:
            ...
            log.info("전처리 CSV 없음 — DB1 원본 데이터 사용 (품질 검증 포함)")
            all_data = []
            offset   = 0
            batch    = 1000
            while True:
            ...
            # ── DB1 co2_in 품질 검증 ─────────────────────────────
            raw_df["co2_in"] = pd.to_numeric(raw_df.get("co2_in"), errors="coerce")
            total_rows   = len(raw_df)
            null_ratio   = raw_df["co2_in"].isna().mean()
            log.info(f"DB1 co2_in Null 비율: {null_ratio:.1%} ({raw_df['co2_in'].isna().sum()}행)")

            if null_ratio > CO2_NULL_RATIO_LIMIT:
                log.warning(
                    f"DB1 co2_in Null 비율 {null_ratio:.1%} > {CO2_NULL_RATIO_LIMIT:.0%} "
                    f"→ 데이터 품질 불량으로 재학습 스킵"
                )
                send_discord_alert(
                    f"⚠️ [재학습 스킵] DB1 co2_in Null 비율 {null_ratio:.1%}\n"
                    f"기준: {CO2_NULL_RATIO_LIMIT:.0%} 초과 → 데이터 품질 불량\n"
                    f"현재 모델 유지"
                )
                return None

            # 이상값 행 제거 (co2_in 범위 밖)
            before = len(raw_df)
            raw_df = raw_df[
                raw_df["co2_in"].isna() |   # Null은 build_train_features에서 처리
                raw_df["co2_in"].between(CO2_VALID_MIN, CO2_VALID_MAX)
            ].reset_index(drop=True)
            removed = before - len(raw_df)
            if removed > 0:
                log.info(f"co2_in 이상값 {removed}행 제거 ({CO2_VALID_MIN}~{CO2_VALID_MAX}ppm 범위 밖)")

            # 최신 N행만 사용 (RETRAIN_RECENT_ROWS > 0 이면)
            if RETRAIN_RECENT_ROWS > 0 and len(raw_df) > RETRAIN_RECENT_ROWS:
                raw_df = raw_df.iloc[-RETRAIN_RECENT_ROWS:].reset_index(drop=True)
                log.info(f"최신 {RETRAIN_RECENT_ROWS}행만 재학습에 사용")
        ...
        # 재학습 모델 유효성: ① 기존 모델보다 나아야 함 ② 최소 R² 0.30 이상
        RETRAIN_MIN_R2 = 0.30   # 이 이하면 데이터 품질 문제로 교체 불가
        if r2_new < RETRAIN_MIN_R2:
            log.warning(
                f"[재학습] 신규 모델 R²={r2_new:.4f} < 최소 기준 {RETRAIN_MIN_R2} "
                f"→ 데이터 품질 문제로 교체 보류"
            )
            send_discord_alert(
                f"⚠️ [재학습 교체 보류 — 품질 문제]\n"
                f"신규 R²={r2_new:.4f} < 최소 기준 {RETRAIN_MIN_R2}\n"
                f"DB1 co2_in 데이터 품질 확인 필요"
            )
            return None
        ...
        if r2_new <= r2_curr:
            log.warning(
                f"[재학습] 신규 모델 성능 미달 "
                f"(R²: {r2_new:.4f} ≤ {r2_curr:.4f}) — Production 교체 보류"
            )
            send_discord_alert(
                f"ℹ️ [재학습 완료 — 교체 보류]\n"
...
class Pipeline:
    ...
    def hourly_step(self):
        ...
        # Step 8: 재학습 트리거 판단
        # ※ RETRAIN_R2_THRESHOLD = -1.0 으로 설정 시 자동 재학습 비활성화
        # ※ 실시간 데이터 품질(DB1 co2_in) 검증 후 임계값 조정 필요
        if metrics is not None:
            trigger = None
            if RETRAIN_R2_THRESHOLD >= 0 and metrics["r2"] < RETRAIN_R2_THRESHOLD:
                trigger = "r2_drop"
            elif metrics["drift_detected"] and RETRAIN_R2_THRESHOLD >= 0:
            ...
            if trigger:
                ...
                if new_model is not None:
                    ...
                    self.ref_co2 = self._load_ref_co2()
                    log.info("✅ 모델 핫스왑 완료")
            elif RETRAIN_R2_THRESHOLD < 0:
                log.info(
                    f"📊 모니터링 전용 모드 (R²={metrics['r2']:.4f}) "
                    f"— 자동 재학습 비활성화 (RETRAIN_R2_THRESHOLD={RETRAIN_R2_THRESHOLD})"
                )
            else:
    ...
    def scheduled_step(self):
        ...
        ※ RETRAIN_R2_THRESHOLD < 0 이면 스킵 (비활성화 모드)
        """
        log.info("=" * 65)
        log.info(f"[SCHEDULED RETRAIN] {datetime.now().isoformat()}")
        if RETRAIN_R2_THRESHOLD < 0:
            log.info("📊 정기 재학습 비활성화 모드 — 스킵 (RETRAIN_R2_THRESHOLD < 0)")
            log.info("[SCHEDULED RETRAIN] 완료 (스킵)")
            return
