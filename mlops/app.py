...
엔드포인트:
  ...
  POST /predict   28개 피처 입력 → CO2 예측값 + 제어 명령 반환
...
app = FastAPI(
    ...
    description="LightGBM 기반 온실 CO2 3시간 후 예측 + 규칙 기반 제어 명령 API",
    version="1.0.0",
)
...
def model_info():
    ...
    return {
        ...
        "target":       "next_co2_in (3시간 후 CO2 ppm)",
    }


@app.post("/predict", summary="CO2 예측 (28개 피처 전체 입력)")
def predict(input_data: CO2Input):
    """
    06강 FastAPI 예측 엔드포인트

    - 입력: 28개 피처 (전처리 완료 상태)
    - 출력: co2_predicted (3시간 후 CO2 ppm) + 제어 명령 요약
    """
    _check_model()

    # Pydantic → dict → DataFrame (FEATURE_COLS 순서 보장)
    ...
    return {
        ...
        "prediction_target": "3시간 후 CO2 농도",
...
def predict_simple(input_data: SimpleSensorInput):
    ...
    row = {
        ...
        "soil_hum_lag3": input_data.soil_hum_lag3 or input_data.soil_hum,
    }
